...
printf 'Setting type annotations of `TorchTrainer` class to match those in "%s"\n' "$input_file"

python -m cdd sync --class "$mod"'/config.py' \
...
for name in ${generate[*]}; do
    fname="$mod"'/'"$name"'.py';
    rm "$fname"
    python -m ml_params_pytorch.ml_params.cdd_cli_gen "$name" | xargs python -m cdd gen;
    git add "$fname"
done
